...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    // --- Use fuzzer input to set PNG parameters ---
    ...
    if (size > current_offset && data[current_offset] > 127) {
        ...
        for (int i = 0; i < num_text_chunks_to_fuzz; ++i) {
            ...
            if (key_end) {
                ...
                key_len = size > current_offset ? size - current_offset : 0;
            }

            char* key_buf = (char*)malloc(key_len + 1);
            if (!key_buf) {
                num_text_chunks_to_fuzz = i; 
                break;
            }
            ...
            context.png_text_chunks[i].text_length = 0; 
            context.png_text_chunks[i].lang = nullptr; 
            context.png_text_chunks[i].lang_key = nullptr;
        }
    ...
    if (context.color_type == PNG_COLOR_TYPE_PALETTE) {
        ...
        // Populate palette entries from fuzzer data such as filling with repeating pattern from input
    ...
    // --- Write the header and info chunks [Important Function - Improvement 1]---
    ...
    if (context.interlace_type == PNG_INTERLACE_ADAM7) {
        // Interlaced: Need all pixel data at once for png_write_image so it's best to imit height/width accordingly.
        ...
        if (total_pixel_data_size > 0) {
              ...
              // [Important Function - Improvement 1]
