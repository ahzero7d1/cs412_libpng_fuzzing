...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  // Error handling block that uses goto for cleanup
  do {
    ...
    WriteBuffer out_buf;
    png_set_write_fn(png_ptr, &out_buf, user_write_data, user_flush_data);
    ...
    row_ptr = png_malloc(png_ptr, rowbytes);
    if (!row_ptr) break;
    ...
    for (uint32_t y = 0; y < height; ++y) {
      ...
      for (size_t x = 0; x < rowbytes; ++x) {
        png_bytep row = static_cast<png_bytep>(row_ptr);
        row[x] = data[input_index++ % size];  // wrap around data
      }
      png_write_row(png_ptr, static_cast<png_bytep>(row_ptr));
    }

    png_write_end(png_ptr, nullptr);

  } while (0);  // Single-iteration do-while for clean error handling
